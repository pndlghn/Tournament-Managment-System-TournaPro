  ...
  <section class="text-center py-42 px-4">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 max-w-6xl mx-auto">   
      ...
      @foreach ($games as $game)
        <a href="/tournaments/{{ $game->slug }}" class="group block overflow-hidden rounded-xl shadow-lg hover:shadow-2xl transition">
          <img src="{{ asset('assets/images/logo_games/'.$game->logo) }}" alt="{{ $game->name }}" class="w-full h-38 object-contain p-8 group-hover:scale-105 transition-transform duration-300">
